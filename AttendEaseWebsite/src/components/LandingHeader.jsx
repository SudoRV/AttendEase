        ...
        <header className="sticky top-0 left-0 right-0 z-50 backdrop-blur-lg bg-white/20 dark:bg-black/30 dark:border-neutral-700 border-t-0 border-l-0 border-r-0 border-solid border-b-2 border-indigo-500">
            ...
            <div className="flex items-center justify-between h-16 max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex items-center gap-4 border-none">
                    ...
                    <button
                        onClick={toggleTheme}
                        className="p-2 rounded-lg bg-slate-100 dark:bg-neutral-700 text-neutral-700 dark:text-white hover:bg-slate-200 dark:hover:bg-neutral-600 transition border-none cursor-pointer"
                    >
                    ...
                    <div className="hidden md:flex gap-3">
...

                        {
                            ...
                            <button
                                ...
                                className={`px-4 py-2 text-white rounded-lg hover:shadow-lg transition text-[15px] font-medium border-none ${!user?.id ? "bg-neutral-700" : "bg-gradient-to-r from-indigo-600 to-indigo-500 !cursor-pointer"}`}
            ...
            <div className="max-w-[1440px] mr-4 px-4 md:mx-auto sm:px-6 lg:px-8 -mb-0.5 overflow-x-auto overflow-y-hidden">
                <div className="items-left justify-left flex gap-2 md:gap-5">
                    {NAV_TABS.map((tab) => (
                        (tab.id !== "dashboard" || user?.id) && (
                            <button
                                key={tab.id}
                                onClick={() => {
                                    navigate(tab.link);
                                }}
                                className={`rounded-t-2xl whitespace-nowrap flex items-center gap-1 px-4 py-2 text-base md:text-[15px] font-medium transition-colors duration-200 cursor-pointer ${location.pathname === tab.link
