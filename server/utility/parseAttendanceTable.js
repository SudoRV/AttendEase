...
const parseAttendanceTable = (month, creds, htmlString, newReq) => {
    ...
    $('tbody tr').each((rowIndex, row) => {
        ...
        const attendance = cells.slice(1, -3).map((i, el) => $(el).text().trim() || "NA").get();

        const leaves = attendance.map(a => ({status: a, count: a.toLowerCase().trim().split(",").filter(a => a.trim() === "l").length})).reduce((prev, current) => {
        ...
        report.leaves = leaves + report?.leaves || 0;
    })
